...
Body_part::Body_part(Body_part_type type) : type{ type } {}

std::string Body_part::describe(const std::string& pronoun)
{
    std::string desc = pronoun + " has " + bodyPartTypeToString(type);
    if (parent) desc += " connected to " + bodyPartTypeToString(parent->type);
    desc += ".";
